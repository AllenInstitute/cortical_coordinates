 ...
 const char * output )
{
  ...
  typename ImageType::Pointer outputImage;
    ...
    itk::idp::BinaryThreshold<ImageType,ImageType>( inputImage, 
                                         static_cast<typename ImageType::PixelType>(lowerThreshold), 
                                         static_cast<typename ImageType::PixelType>(upperThreshold),
                                         static_cast<typename ImageType::PixelType>(insideValue), 
...
int main( int argc, char *argv[] )
{
  if (argc < 7 )
    ...
    std::cout << "input lowerThreshold upperThreshold insideValue outsideValue output [format] [dimension]";
    std::cout << std::endl;
    return EXIT_FAILURE;
    }
  ...
  double lowerThreshold = atof( argv[2] );
  double upperThreshold = atof( argv[3] );
  double insideValue = atof( argv[4] );
  double outsideValue = atof( argv[5] );  
  ...
  if( itksys::SystemTools::Strucmp( format.c_str(), "uchar" ) == 0 && dimension == 2 )
    {
    err = Pipeline<UCHARImageType>( input.c_str(), lowerThreshold, upperThreshold, insideValue, outsideValue, 
                                    output.c_str() );
    if ( err ) return EXIT_FAILURE;
    }
  else if( itksys::SystemTools::Strucmp( format.c_str(), "ushort" ) == 0 && dimension == 2 )
    {
    err = Pipeline<USHORTImageType>( input.c_str(), lowerThreshold, upperThreshold, insideValue, outsideValue, 
                                    output.c_str() );
    if ( err ) return EXIT_FAILURE;
    }
  else if( itksys::SystemTools::Strucmp( format.c_str(), "uint" ) == 0 && dimension == 2 )
    {
    err = Pipeline<UINTImageType>( input.c_str(), lowerThreshold, upperThreshold, insideValue, outsideValue, 
                                    output.c_str() );
    if ( err ) return EXIT_FAILURE;
    }
  else if( itksys::SystemTools::Strucmp( format.c_str(), "uchar" ) == 0 && dimension == 3 )
    {
    err = Pipeline<UCHARVolumeType>( input.c_str(), lowerThreshold, upperThreshold, insideValue, outsideValue, 
                                    output.c_str() );
    if ( err ) return EXIT_FAILURE;
    }
  else if( itksys::SystemTools::Strucmp( format.c_str(), "ushort" ) == 0 && dimension == 3 )
    {
    err = Pipeline<USHORTVolumeType>( input.c_str(), lowerThreshold, upperThreshold, insideValue, outsideValue, 
                                    output.c_str() );
    if ( err ) return EXIT_FAILURE;
    }
  else if( itksys::SystemTools::Strucmp( format.c_str(), "uint" ) == 0 && dimension == 3 )
    {
    err = Pipeline<UINTVolumeType>( input.c_str(), lowerThreshold, upperThreshold, insideValue, outsideValue, 
                                    output.c_str() );
    if ( err ) return EXIT_FAILURE;
    }
  else if( itksys::SystemTools::Strucmp( format.c_str(), "float" ) == 0 && dimension == 3 )
    {
    err = Pipeline<FLOATVolumeType>( input.c_str(), lowerThreshold, upperThreshold, insideValue, outsideValue, 
                                    output.c_str() );
    if ( err ) return EXIT_FAILURE;
    }
  else 
