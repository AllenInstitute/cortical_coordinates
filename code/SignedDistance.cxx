 ...
 double maxDistance )
{
  ...
  typename ImageType::Pointer signedDistance;

    itk::idp::ReadImage<ImageType>( input, inputImage );
    itk::idp::BinaryThreshold<ImageType,ImageType>( inputImage, 1, itk::NumericTraits<PixelType>::max() ,-1.0, 1.0, signedDistance);
    itk::idp::SignedDistance<ImageType>( signedDistance, signedDistance, maxDistance );
    itk::idp::WriteImage<ImageType>( output , signedDistance );
...
int main( int argc, char *argv[] )
{
  if (argc < 3 )
    ...
    std::cout << "input output [format] [dimension]";
    std::cout << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if( itksys::SystemTools::Strucmp( format.c_str(), "uchar" ) == 0 && dimension == 2 )
    {
    err = Pipeline<UCHARImageType>( input.c_str(), output.c_str(), maxDistance );
    if ( err ) return EXIT_FAILURE;
    }
  else if( itksys::SystemTools::Strucmp( format.c_str(), "ushort" ) == 0 && dimension == 2 )
    {
    err = Pipeline<USHORTImageType>( input.c_str(), output.c_str(), maxDistance );
    if ( err ) return EXIT_FAILURE;
    }
  else if( itksys::SystemTools::Strucmp( format.c_str(), "uint" ) == 0 && dimension == 2 )
    {
    err = Pipeline<UINTImageType>( input.c_str(), output.c_str(), maxDistance );
    if ( err ) return EXIT_FAILURE;
    }
  else if( itksys::SystemTools::Strucmp( format.c_str(), "uchar" ) == 0 && dimension == 3 )
    {
    err = Pipeline<UCHARVolumeType>( input.c_str(), output.c_str(), maxDistance );
    if ( err ) return EXIT_FAILURE;
    }
  else if( itksys::SystemTools::Strucmp( format.c_str(), "ushort" ) == 0 && dimension == 3 )
    {
    err = Pipeline<USHORTVolumeType>( input.c_str(), output.c_str(), maxDistance );
    if ( err ) return EXIT_FAILURE;
    }
  else if( itksys::SystemTools::Strucmp( format.c_str(), "uint" ) == 0 && dimension == 3 )
    {
    err = Pipeline<UINTVolumeType>( input.c_str(), output.c_str(), maxDistance );
    if ( err ) return EXIT_FAILURE;
    }
  else if( itksys::SystemTools::Strucmp( format.c_str(), "float" ) == 0 && dimension == 3 )
    {
    err = Pipeline<FLOATVolumeType>( input.c_str(), output.c_str(), maxDistance );
    if ( err ) return EXIT_FAILURE;
    }
  else 
    {
